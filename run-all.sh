...
for i in {1..$runs}; do
  for nthreads in "${threads[@]}"; do
    # Birth Analysis
    python birth_analysis_grizzly.py -t $nthreads -f $HOME/composer/python/benchmarks/datasets/birth_analysis/_data/babynames.txt >> results/birth_analysis/weld.stdout 2>> results/birth_analysis/weld.stderr 
